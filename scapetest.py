...
def getRecipe(url):
    ...
    try:
        ...
        for recipe in h3list:
            ...
            print(created_date.strftime('%Y-%m-%d') + "|" + str(diff.days) + "|" + getVisit(recipe_url) + "|" + getBookmark(recipe_url) + "|" + recipe.getText() + "|" + recipe.find("a").attrs['href'])
            writer.writerow((created_date.strftime('%Y-%m-%d'),str(diff.days),getVisit(recipe_url),getBookmark(recipe.find("a").attrs['href']),recipe.getText(),recipe.find("a").attrs['href']))
